    ...
    try {
      ...
      if (retryCount < this.MAX_RETRIES) {
        try {
          ...
          this.socketGateway.sendResponse(data.senderId, { status: "failed", message: `Message ${action} request failed after 5 retries. Error: ${error}` });
        }
      }
        ...
        try {
          ...
          // producer failure after 5 retries
          this.socketGateway.sendResponse(data.senderId, { status: "failed", message: `Message ${action} request failed after ${this.MAX_RETRIES} retries. Error: ${error}` });
        }
      }
    }
  }
